...
# Network name
network_name = "jatkasaari_network"

# Budget values for sensitivity analysis
#budgets = range(0,200001,10000)
#budgets = range(0,2_000_001,50_000)
...
try:
    ...
    for i, B_val in enumerate(budgets):
        ...
        if status in ['optimal', 'solved', 'feasible']:
            ...
            merged_od.to_csv("./output/od_variables.csv", index=False)
            ...
            if link_variables:
                ...
                merged_links.to_csv("./output/link_variables.csv", index=False)
    ...
    if sensitivity_results:
        ...
        for col in all_results.columns:
            if col != "OD":
                all_results[col] = pd.to_numeric(all_results[col], errors="ignore")
